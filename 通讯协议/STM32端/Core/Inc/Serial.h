...
//ROS2发送代码
void Pack_Data_ROS2(_send_packetinfo sendinfo,uint8_t* feedArray);//ROS2下的打包函数
int CDC_Receive_ROS2(uint8_t* Buf, uint16_t Len,_receive_packetinfo packinfo);//ROS2下的接收函数
void Data_Select(uint8_t* Origin_Data,uint8_t* Finish_Data);//ROS2下的数据挑选函数
uint16_t Get_CRC16_Check_Sum(const uint8_t * pchMessage, uint32_t dwLength, uint16_t wCRC);//校验码生成
#endif
